...
class Task:
    ...
    def print_tasks_table(data):
        ...
        col_widths = {key: max(len(str(row.get(key, '')))
                               for row in data) for key in headers}
        # Considera o tamanho do cabeçalho
        col_widths = {key: max(col_widths[key], len(key)) for key in headers}

        # Imprime o cabeçalho
        header_row = " | ".join(
            f"{key.ljust(col_widths[key])}" for key in headers)
        print_with_color_tags(
            f"<black><bg_white> {header_row} </bg_white></black>")
        # print(header_row)
        ...
        for row in data:
            print('', " | ".join(
                f"{str(row.get(key, '')).ljust(col_widths[key])}" for key in headers), ' ')

    def show_tasks():
        tasks = Task.TASKS
        Task.print_tasks_table(
            [{"name": k, **{x: v[x] for x in v if x != "function"}} for k, v in tasks.items()])
        return
