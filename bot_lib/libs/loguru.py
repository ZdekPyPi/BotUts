...
def elapsed_time():
    ...
    return round(
        (time() if task_time_end is None else task_time_end) - task_time_start, 2)
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            if BEFORE_START_EVENT:
                [fn() for fn in BEFORE_START_EVENT]
            ...
            if AFTER_FINISH_EVENT:
                [fn("WARNING" if logger.HAS_WARNING else "SUCCESS", "")
                    for fn in AFTER_FINISH_EVENT]
            ...
            logger.log(
                ...
                depth=3)
            pass
        # except ValueError       : logger.error("FINISHED",func_name =
        # function.__name__)
        except Exception as e:
            if AFTER_FINISH_EVENT:
                [fn("CRITICAL", str(e)) for fn in AFTER_FINISH_EVENT]
            logger.critical("FINISHED", func_name=function.__name__, depth=3)
        # QUANDO FOR UM EVENTO QUE DEU ERRO NAO PODE CHAMAR OS EVENTOS
        # NOVAMENTE SE CAIR EM EXCEPTION
