...
def validade_portainer_credentials():
    ...
    # os.environ["PORTAINER_PASSWORD"] = "Qwerty123!@#"  # Substitua pela sua senha do Portainer
    ...
    if any([not portainer_host,not portainer_user, not portainer_password]):
        print_with_color_tags(f"<bg_red> PARA USAR O PORTAINER CONFIGURE AS VARIAVEIS DE AMBIENTE CORRETAMENTE </bg_red>")
        status = (False,None)
    print()
    if not portainer_host:
        print_with_color_tags(f" PORTAINER_HOST: \t<bg_red> NOT FOUND </bg_red>")
    if not portainer_user:
        print_with_color_tags(f" PORTAINER_USER: \t<bg_red> NOT FOUND </bg_red>")
    if not portainer_password:
        print_with_color_tags(f" PORTAINER_PASSWORD: \t<bg_red> NOT FOUND </bg_red>")
...
def redeploy_stack(stack, token):
    portainer_host     = os.getenv("PORTAINER_HOST")
    url = f"{portainer_host}/api/stacks/{stack['Id']}/git/redeploy?endpointId={stack['EndpointId']}"
    headers = {
    ...
    if response.status_code != 200:
        raise Exception(f"Falha ao redeployar stack {stack['Name']}: {response.status_code} - {response.text}")
