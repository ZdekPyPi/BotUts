...
def deletar_sessao(session_id):
    ...
    try:
        ...
        print_with_color_tags(
            f"<bg_green>Removendo sessão {session_id}: {
                resp.status_code}</bg_green>")
    except Exception as e:
        print_with_color_tags(
            f"<bg_red>Erro ao remover sessão {session_id}: {e}</bg_red>")
...
def deletar_todas():
    ...
    if not sessao_ids:
        return print_with_color_tags(
            f"<black><bg_white>Nenhuma sessão ativa encontrada</bg_white></black>")

    print_with_color_tags(
        f"<bg_blue>{
            len(sessao_ids)} sessões encontradas. Iniciando remoção...</bg_blue>")
