...
class configLogger:
    ...
    def rename_rotated_file(filepath):
        new_path = os.path.dirname(filepath) + \
            f"/{now(fmt='%Y-%m-%d_T_%H-%M-%S')}.log"
        os.rename(filepath, new_path)

    # ===================== FORMATO DE MENSAGEM
    @staticmethod
    def formatter(record):
        def extraReplace(key, key2, tam): return f"{{extra[{key}]: ^{tam}}}" if record.get(
            "extra") and record.get("extra").get(key) else f"{{{key2}: ^{tam}}}"

        def extraOpt(key, tam): return f"{{extra[{key}]: ^{tam}}}" if record.get(
            "extra") and record.get("extra").get(key) else f"{' ' * tam}"
    ...
    def re_start(self):
        open(self.logger_path, "a").write(f"#{'=' * 200}\n")
...
class configLevels:
    ...
    def __init__(self):
        ...
        self.log.level("INTER", no=30, color="<cyan><bold>")
